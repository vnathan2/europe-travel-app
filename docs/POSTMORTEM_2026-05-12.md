...
- **El fallback offline de Lady salvó la cara durante la rotación**. Las FAQ pre-canned en `utils/offline_faqs.py` no se activaron (porque Gemini fallaba con timeout, no con excepción del SDK), pero conceptualmente este incidente justifica su existencia. Hay un pendiente: hacer que el except de `obtener_respuesta` también capture timeouts/retry errors, no solo excepciones genéricas.
...
- Si Gemini cae nuevamente y el fallback no se activa, revisar si el except en `obtener_respuesta` captura `google.api_core.exceptions.RetryError` y `RetryError`.
